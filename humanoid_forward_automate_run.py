...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', help='environment ID', default='DartHumanSwim-v0')
    ...
    parser.add_argument('--data_collect_env', help='Environment used to collect data', default='DartHumanSwim-v0')
    ...
    norm_scale = np.array([2, np.pi, 2, 50, 18, np.pi, 18, 50])
    norm_scale_str = ''
    for i in norm_scale:
    ...
    for i in range(0, 6, 1):
        ...
        train_policy = subprocess.call(
            'OMP_NUM_THREADS="1" mpirun -np ' + str(
                cpu_count) + ' python ./running_regimes/humanoid_more_info_two_objs_mirror_policy_train.py'
            + ' --env ' + args.env
            + ' --seed ' + str(seed)
            + ' --curr_run ' + curr_run
