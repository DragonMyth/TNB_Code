...
class DartHumanoidSwimStraightUpperEnforceEnv(dart_env.DartEnv, utils.EzPickle):
    def __init__(self):
        control_bounds = np.array([[1.0] * 14, [-1.0] * 14])
        self.action_scale = np.pi/2
        self.torque_scale = 0.03
        self.frame_skip = 5
        dart_env.DartEnv.__init__(self, 'humanoid_swimmer_upper.skel', self.frame_skip, 33, control_bounds, dt=0.002,
...
class DartHumanoidSwimStraightUpperReleaseEnv(DartHumanoidSwimStraightUpperEnforceEnv):
    def __init__(self):
        DartHumanoidSwimStraightUpperEnforceEnv.__init__(self)
